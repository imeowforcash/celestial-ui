...
const ScriptCard: React.FC<ScriptCardProps> = React.memo(({ script, onClick, isBlox = false }) => {
  const isRScriptsTrending = (s: any): s is TrendingScript => {
    return !isBlox && 'script' in s && s.script && typeof s.script === 'object' && 'title' in s.script;
  };
  ...
  const isPresetScript = isPreset(script);
  ...
  if (isPresetScript) {
    ...
    likes = (s as any).likeCount || 0;
    dislikes = (s as any).dislikeCount || 0;
    views = s.views || 0;
    createdAt = s.createdAt;
  } else if (isRScriptsTrending(script)) {
  ...
  const [isVisible, setIsVisible] = React.useState(() => isPresetScript || hasThumbnailCached(imgSrc));
  ...
  }, [hasRemoteImage, imgSrc, isPresetScript]);
    ...
    >
      <div className="w-full aspect-video overflow-hidden">
        <img 
          ...
          loading={displaySrc === placeholderImg || isPresetScript || isVisible || hasThumbnailCached(imgSrc) ? "eager" : "lazy"}
          fetchPriority={isPresetScript ? "high" : "auto"}
          onError={handleError}
        />
      </div>
