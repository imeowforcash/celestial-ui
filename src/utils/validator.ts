    ...
    try {
      ...
      if (err && typeof err.line === 'number') {
        errors.push({
          row: err.line - 1,
          column: err.column,
          text: err.message.replace(/^[\[\d+:\d+\]]\s*/, ''),
          type: 'error'
        });
      }
    }
    ...
    lines.forEach((line, row) => {
      ...
      if (!trimmed.startsWith('--')) {
        ...
        if (singleQuotes % 2 !== 0 || doubleQuotes % 2 !== 0) {
           if (!trimmed.startsWith('--') && !errors.some(e => e.row === row)) {
             errors.push({
               row,
               column: line.length,
      ...
      if (/^(if|elseif)\b/.test(trimmed) && !/\bthen\b/.test(trimmed) && !trimmed.endsWith('then')) {
         if (!trimmed.startsWith('--') && !errors.some(e => e.row === row)) {
      ...
      if (/^(while|for)\b/.test(trimmed) && !/\bdo\b/.test(trimmed) && !trimmed.endsWith('do')) {
         if (!trimmed.startsWith('--') && !errors.some(e => e.row === row)) {
           const lastTokenCol = this.getLastTokenIndex(line);
           errors.push({
             row,
