...
const Console: React.FC<ConsoleProps> = ({ onKeepMountedChange, onWatchingChange, isStandalone = false }) => {
  ...
  }, [isStandalone, logs, isWatching, stopWatcher]);
  ...
  }, [logs.length, autoScroll, listRef]);
  ...
  useEffect(() => {
    ...
    const checkInterval = setInterval(() => {
      if (listRef.current && listRef.current.element) {
        listRef.current.element.addEventListener('scroll', checkScroll, { passive: true });
    ...
    return () => {
      clearInterval(checkInterval);
      if (listRef.current && listRef.current.element) {
        listRef.current.element.removeEventListener('scroll', checkScroll);
      }
    };
  }, [logs.length > 0]);
  ...
  return (
      <div className={editorStyles["editor-container"]}>
        ...
        {contextMenu.visible && (
          <div
            ...
            style={{
              ...
              top: Math.min(contextMenu.y, window.innerHeight - (isStandalone && contextMenu.type === 'log' ? 120 : 60)),
              zIndex: 1000,
            }}
          >
