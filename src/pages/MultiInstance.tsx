  ...
  const handleSelfHeal = useCallback(async () => {
    ...
    try {
      ...
      if (restoredAccounts.length > 0) {
        ...
        toast.error("No accounts found to recover", { duration: 2000 });
      }
    } catch {
    } finally {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      unlistenAdded = await listen<Account>("browser-login-account-added", (event) => {
        ...
        toast.success("Account added from browser login", { duration: 2000 });
      });

      unlistenError = await listen<string>("browser-login-error", () => {
      });
    };
