...
import { CopyIcon, PasteIcon, RobloxIcon, SplitviewIcon } from "../../../assets/Icons";
import { cn } from "@/utils/ui";
import { useFontSettings, useKeybindSettings, useUiSettings } from "../../../contexts/SettingsContext";
...
type ValidatorInstance = InstanceType<validatorModule["validator"]>;
...
let completionBucketsCache: CompletionBuckets | null = null;
let monacoContribPromise: Promise<void> | null = null;

const MARKER_OWNER = "celestial-lua-lint";
const COMPLETION_USAGE_COMMAND_ID = "celestial.recordCompletionUsage";
const MAX_COMPLETION_SUGGESTIONS = 200;

type DefItem = LuaDefinitionsModule["definitions"][number];
type CompletionTemplate = Omit<Monaco.languages.CompletionItem, "range">;
type CompletionBuckets = {
  usageVersion: number;
  plain: CompletionTemplate[];
  dot: CompletionTemplate[];
  colon: CompletionTemplate[];
};
...
const loadMonacoContrib = (): Promise<void> => {
  if (!monacoContribPromise) {
    monacoContribPromise = Promise.all([
      import("monaco-editor/esm/vs/editor/contrib/find/browser/findController.js"),
      import("monaco-editor/esm/vs/editor/contrib/suggest/browser/suggestController.js"),
      import("monaco-editor/esm/vs/editor/contrib/snippet/browser/snippetController2.js"),
    ]).then(() => {});
  }
  return monacoContribPromise;
};

const getValidator = async (): Promise<ValidatorInstance> => {
  if (validatorInstance) {
    return validatorInstance;
  }

  const { validator: Validator } = await loadValidatorModule();
...
const buildCompletionTemplates = (
  monaco: typeof Monaco,
  items: DefItem[],
): CompletionTemplate[] => (
...
const getCompletionBuckets = (
  monaco: typeof Monaco,
  defsModule: LuaDefinitionsModule,
): CompletionBuckets => {
  const usageVersion = defsModule.getUsageVersion();

  if (completionBucketsCache && completionBucketsCache.usageVersion === usageVersion) {
    return completionBucketsCache;
  }

  const defs = defsModule.getDefs();

  const plainDefs = defs
    .filter((item) => item.meta !== "Method" && item.meta !== "Property")
  ...
  const dotDefs = defs
    ...
    .sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
  const colonDefs = defs
    .filter((item) => item.meta === "Method")
    .sort((a, b) => (b.score ?? 0) - (a.score ?? 0));

  completionBucketsCache = {
    usageVersion,
    plain: buildCompletionTemplates(monaco, plainDefs),
    dot: buildCompletionTemplates(monaco, dotDefs),
    colon: buildCompletionTemplates(monaco, colonDefs),
  };
  ...
  if (!completionDisposable) {
    completionDisposable = monaco.languages.registerCompletionItemProvider("lua", {
      ...
      provideCompletionItems: async (model, position) => {
        ...
        const buckets = getCompletionBuckets(monaco, defsModule);
        const templates = triggerChar === ":"
          ? buckets.colon
          : triggerChar === "."
            ? buckets.dot
            : buckets.plain;
...
const CodeEditor: React.FC<CodeEditorProps> = memo(({
  ...
  const scrollbarSuppressionRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const editorLineHeight = useMemo(
    () => Math.max(16, Math.round(editorFontSize * 1.2)),
    [editorFontSize],
  );
  ...
  const handleEditorMount: OnMount = useCallback((editor, monaco) => {
    ...
    const hideScrollbarBriefly = () => {
      hideScrollbarRef.current = true;
      hideScrollbarImmediately();

      if (scrollbarSuppressionRef.current) {
        clearTimeout(scrollbarSuppressionRef.current);
      }

      scrollbarSuppressionRef.current = setTimeout(() => {
        hideScrollbarRef.current = false;
        scrollbarSuppressionRef.current = null;
      }, 120);
    };
    ...
    const layoutDispose = editor.onDidLayoutChange(() => {
      hideScrollbarBriefly();
    });

    const scrollDisposable = editor.onDidScrollChange(() => {
      if (pointerInEditorRef.current && !hideScrollbarRef.current) {
        showScrollbarTemporarily();
      }
    });
  ...
  useEffect(() => {
    if (!isMonacoReady || !isEditorActive) {
      return;
    }
    ...
    const refreshEditor = () => {
      ...
      if (!editor || !editorShell) {
        frameId = null;
        return;
      }

      if (editorShell.getClientRects().length === 0) {
        frameId = requestAnimationFrame(refreshEditor);
        return;
      }
  ...
  useEffect(() => {
    ...
    const refreshEditor = () => {
      ...
      frameId = requestAnimationFrame(() => {
        const editor = monacoEditorRef.current;
        const editorShell = editorShellRef.current;
        if (!editor || !editorShell || editorShell.getClientRects().length === 0) {
          frameId = null;
          return;
        }
    ...
    window.addEventListener("focus", refreshEditor);
    document.addEventListener("visibilitychange", refreshEditor);

    return () => {
      window.removeEventListener("focus", refreshEditor);
      document.removeEventListener("visibilitychange", refreshEditor);
      if (frameId !== null) {
        cancelAnimationFrame(frameId);
      }
    };
  ...
  }, [editorBehaviorOptions, editorFontFamily, editorFontSize, editorLineHeight, readOnly]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const runValidation = async () => {
        try {
          const cachedValidator = await getValidator();
          if (cancelled) {
            return;
          }

          const newAnnotations = cachedValidator.validate(code);
  ...
  }), [editorBehaviorOptions, readOnly]);
  ...
  const handleOpenRoblox = useCallback(() => {
    invoke("open_roblox").catch(() => {});
  }, []);

  const showAccountsButton = !hideMultiInstanceButton;
  const showHistoryButton = !disableHistory;
  const showRobloxButton = !hideRobloxButton;
  const buttonRight = useMemo(() => {
    ...
    if (showAccountsButton) {
      positions.accounts = right;
      right += 56;
    }

    if (showRobloxButton) {
      positions.roblox = right;
      right += 56;
    }

    if (showHistoryButton) {
      positions.history = right;
    }

    return positions;
  }, [showAccountsButton, showHistoryButton, showRobloxButton]);
        ...
        <>
          ...
          {showHistoryButton && (
            <button
              ...
              onClick={onToggleHistoryPanel}
              style={{ right: buttonRight.history }}
            >
              <HistoryIcon />
            </button>
          )}
          ...
          {showRobloxButton && (
            <button
              className={styles["roblox-button"]}
              onClick={handleOpenRoblox}
              style={{ right: buttonRight.roblox }}
            >
              <RobloxIcon />
            </button>
          )}
          ...
          {showAccountsButton && (
            <button
              ...
              onClick={onToggleAccountsPanel}
              style={{ right: buttonRight.accounts }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" />
                <path d="M16 3.128a4 4 0 0 1 0 7.744" />
