...
import { useKeybindSettings, useRuntimeSettings, useSettingsLoading, useUiSettings } from "./contexts/SettingsContext";
...
function App() {
  ...
  const [isConsoleWatching, setIsConsoleWatching] = useState(false);
  const [librarySearchQuery, setLibrarySearchQuery] = useState<string>("");
  const [mountNonEditorViews, setMountNonEditorViews] = useState(false);
  const [isEditorReady, setIsEditorReady] = useState(false);
  
  const { appTitle, currentTheme, sidebarVisible, setSidebarVisible, toggleSidebar } = useUiSettings();
  const { autoHideSidebar, discordRpcEnabled, preventUpdates } = useRuntimeSettings();
  ...
  }, [checkDocumentsAccess, isFocused, isLoading]);
  ...
  useEffect(() => {
    ...
    if (!discordRpcEnabled) {
      invoke('clear_rpc_presence').catch(() => {});
      return;
    }
    ...
    const updatePresence = async () => {
      try {
        ...
        await invoke('update_rpc_presence', {
          view: activeView,
          fileName: currentFileName,
          appVersion: version,
          isWatching: isConsoleWatching,
  ...
  }, [activeView, currentFileName, isConsoleWatching, librarySearchQuery, discordRpcEnabled, getAppVersion, isLoading]);

  useEffect(() => {
    const unlisten = listen<boolean>('pip-watching-changed', (event) => {
      setIsConsoleWatching(event.payload);
    });
  ...
  useEffect(() => {
    ...
    void appWindow.onDragDropEvent(async (event) => {
      ...
      try {
        ...
        toast.error("Failed to import dropped scripts", { duration: 2500 });
      }
    }).then((unlisten) => {
      if (disposed) {
        unlisten();
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      if (!sidebarVisible && event.clientX <= SIDEBAR_REVEAL_EDGE_PX) {
        setSidebarVisible(true);
        return;
      }

      if (sidebarVisible && event.clientX > SIDEBAR_HIDE_THRESHOLD_PX) {
  ...
  }, [autoHideSidebar, setSidebarVisible, sidebarVisible]);
  ...
  return (
    <div 
      className={cn(shellStyles["app-container"], !isFocused && shellStyles.unfocused, isUiLoading && shellStyles["app-loading"])} 
      data-theme={currentTheme.toLowerCase().replace(/\s+/g, '-')}
        ...
        <div className={shellStyles["main-layout"]}>
            ...
            <AnimatePresence initial={false}>
              {sidebarVisible && (
                <m.div
                  ...
                  animate={{ width: 60, marginRight: 12 }}
                  exit={{ width: 0, marginRight: 0 }}
                  transition={{ duration: 0.3, ease: [0.32, 0.72, 0, 1] }}
                  style={{ overflow: 'visible', display: 'flex', flexDirection: 'column', minWidth: 0 }}
                >
                  <m.aside
          ...
          <div style={{ display: activeView === 'editor' ? 'contents' : 'none' }}>
            <Editor 
              ...
              isActive={activeView === 'editor'}
              onCurrentFileChange={setCurrentFileName}
            ...
            <Suspense fallback={null}>
              <div style={{ display: activeView === 'console' ? 'contents' : 'none' }}>
                <Console 
                  ...
                  onWatchingChange={setIsConsoleWatching}
                />
              </div>
            </Suspense>
          )}
