...
#[cfg(target_os = "macos")]
static MACOS_VERSION_CACHE: Mutex<Option<u8>> = Mutex::new(None);
static LOGIN_IMPORT_RUNNING: AtomicBool = AtomicBool::new(false);
static TRAY_ICON_DISABLED_STATE: AtomicU8 = AtomicU8::new(0);
static TRAY_FOLDER_HIDDEN_STATE: AtomicU8 = AtomicU8::new(0);
static TRAY_MONITOR_RUNNING: AtomicBool = AtomicBool::new(false);
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        .on_page_load(move |window, payload| {
            ...
            tauri::async_runtime::spawn(async move {
                ...
                LOGIN_IMPORT_RUNNING.store(false, Ordering::SeqCst);
            });
        })
        .build()
        .map_err(|_| String::new())?;
...
#[cfg(target_os = "macos")]
fn set_tray_icon_disabled_state(disabled: bool) {
    TRAY_ICON_DISABLED_STATE.store(if disabled { 2 } else { 1 }, Ordering::SeqCst);
}

#[cfg(target_os = "macos")]
fn set_runtime_tray_folder_hidden(hidden: bool) {
    TRAY_FOLDER_HIDDEN_STATE.store(if hidden { 2 } else { 1 }, Ordering::SeqCst);
}

#[cfg(target_os = "macos")]
fn tray_icon_disabled() -> bool {
    match TRAY_ICON_DISABLED_STATE.load(Ordering::SeqCst) {
        1 => false,
        2 => true,
        _ => true,
    }
...
#[cfg(target_os = "macos")]
fn tray_folder_hidden() -> bool {
    match TRAY_FOLDER_HIDDEN_STATE.load(Ordering::SeqCst) {
        1 => false,
        2 => true,
        _ => true,
    }
...
#[cfg(target_os = "macos")]
fn tray_scripts_enabled() -> bool {
    !tray_icon_disabled() && !tray_folder_hidden()
}

#[cfg(target_os = "macos")]
fn start_tray_monitor(app_handle: tauri::AppHandle) {
    ...
    std::thread::spawn(move || {
        ...
        loop {
            ...
            if !tray_scripts_enabled() {
                TRAY_MONITOR_RUNNING.store(false, Ordering::SeqCst);
                break;
            }
...
#[cfg(target_os = "macos")]
fn toggle_window_fullscreen(app: &tauri::AppHandle) {
...
fn set_tray_icon_disabled(app: tauri::AppHandle, disabled: bool) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        set_tray_icon_disabled_state(disabled);
        if disabled {
            let _ = app.remove_tray_by_id("menu-bar");
        } else {
            create_menu_bar_tray_icon(&app).map_err(|_| String::new())?;
            if !tray_folder_hidden() {
                start_tray_monitor(app);
            }
        }
    }
...
#[tauri::command]
fn set_tray_folder_hidden(app: tauri::AppHandle, hidden: bool) -> Result<(), String> {
    #[cfg(target_os = "macos")]
    {
        set_runtime_tray_folder_hidden(hidden);

        if app.tray_by_id("menu-bar").is_none() {
            return Ok(());
        }

        update_menu_bar_tray_menu(&app)?;

        if !hidden && !tray_icon_disabled() {
            start_tray_monitor(app);
        }
    }
...
fn get_default_scripts_dir() -> Result<PathBuf, String> {
...
fn write_script_to_default_location(
    ...
    let target_dir = get_default_scripts_dir()?;
...
#[cfg(target_os = "macos")]
#[tauri::command]
fn get_macos_version() -> Result<u8, String> {
    if let Ok(cache) = MACOS_VERSION_CACHE.lock() {
    ...
    let major = u8::try_from(version.majorVersion).map_err(|_| String::new())?;

    if let Ok(mut cache) = MACOS_VERSION_CACHE.lock() {
        *cache = Some(major);
    }
...
#[cfg(target_os = "macos")]
fn read_tray_settings() -> (bool, bool) {
    ...
    let disable_tray_icon = settings_json
        .get("disableTrayIcon")
        .and_then(|value| value.as_bool())
        .unwrap_or(true);
    let hide_tray_folder = settings_json
        .get("hideTrayFolder")
        .and_then(|value| value.as_bool())
        .unwrap_or(true);

    (disable_tray_icon, hide_tray_folder)
}
...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            tauri::async_runtime::spawn(async move {
                let _ = tauri::async_runtime::spawn_blocking(history::init_history).await;
                ...
                #[cfg(target_os = "macos")]
                {
                    let (disable_tray_icon, hide_tray_folder) =
                        match tauri::async_runtime::spawn_blocking(read_tray_settings).await {
                            Ok(settings) => settings,
                            Err(_) => (true, true),
                        };

                    set_tray_icon_disabled_state(disable_tray_icon);
                    set_runtime_tray_folder_hidden(hide_tray_folder);

                    if !disable_tray_icon {
                        let _ = create_menu_bar_tray_icon(&app_handle);
                        if !hide_tray_folder {
                            start_tray_monitor(app_handle.clone());
                        }
                    }
                }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            save_hydrogen_key,
            get_key_expiration,
            hubble::is_hubble_ready,
            hubble::ensure_hubble_ready,
            hubble::filter_hubble_candidates,
            download_script,
            save_script,
