...
fn get_bridge_path(executor: ExecutorKind) -> Result<PathBuf, String> {
    ...
    let path = match executor {
        ExecutorKind::Hydrogen => home.join("Hydrogen").join("workspace/celestial_multiexec.json"),
        ExecutorKind::Opiumware => home.join("Opiumware").join("workspace/celestial_multiexec.json"),
...
#[tauri::command]
pub fn get_bridge_clients(executor: Option<String>) -> Result<Vec<Client>, String> {
    let executor = normalize_executor(executor.as_deref());
    let data = read_bridge(executor);
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs())
        .unwrap_or(0);
    let active: Vec<Client> = data
        .clients
        .into_iter()
        .filter(|c| now - c.last_heartbeat <= 15)
        .collect();
    Ok(active)
}
